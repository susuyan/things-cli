...
pub fn handle_command(cli: Cli) -> anyhow::Result<()> {
    match cli.command {
        Commands::Todo(cmd) => todo::handle(cmd, &cli.global, cli.global.json),
        Commands::Project(cmd) => project::handle(cmd, &cli.global, cli.global.json),
        Commands::Area(cmd) => area::handle(cmd, &cli.global, cli.global.json),
        ...
        Commands::List(cmd) => list::handle(cmd, cli.global.json),
